...
![image-20211226204935748](https://gcore.jsdelivr.net/gh/Kele-Bingtang/static/img/Redis/20211226205002.png)
...
![image-20210409112108209](https://gcore.jsdelivr.net/gh/Kele-Bingtang/static/img/Redis/20211226204510.png)
...
![image-20210409115138265](https://gcore.jsdelivr.net/gh/Kele-Bingtang/static/img/Redis/20211226204641.png)

![image-20210409115114373](https://gcore.jsdelivr.net/gh/Kele-Bingtang/static/img/Redis/20211226204814.png)

![image-20210409115241761](https://gcore.jsdelivr.net/gh/Kele-Bingtang/static/img/Redis/20211226204857.png)
...
![在这里插入图片描述](https://gcore.jsdelivr.net/gh/Kele-Bingtang/static/img/Redis/20211226204911.png)
...
- 启动好 3 个不同端口服务后，我们再分别开启 Redis 连接
    ...
    ![image-20210409134833129](https://gcore.jsdelivr.net/gh/Kele-Bingtang/static/img/Redis/20211226205444.png)
...
- 配置为一个 Master 和 两个 Slave（即一主二从）
    ...
    ![image-20210409134929416](https://gcore.jsdelivr.net/gh/Kele-Bingtang/static/img/Redis/20211226205520.png)

- 在主机设置值，在从机都可以取到，但是从机不能写值

    ![image-20210409135325865](https://gcore.jsdelivr.net/gh/Kele-Bingtang/static/img/Redis/20211226205532.png)

    我们这里是使用命令搭建，是「暂时的」，如果重启三个 Redis 服务，则又恢复到三主的地位

    如果想配置「永久的」，则去配置里进行修改，找到 `slaveof <ip> <port>` 指令进行配置：
    
    ![image-20210409135633320](https://gcore.jsdelivr.net/gh/Kele-Bingtang/static/img/Redis/20211226205703.png)
...
![image-20211226210450248](https://gcore.jsdelivr.net/gh/Kele-Bingtang/static/img/Redis/20211226210451.png)

在一个从机用 `slaveof <ip> <port>` 指令连接另一个从机。

![](https://gcore.jsdelivr.net/gh/Kele-Bingtang/static/img/Redis/20211226210356.png)

![](https://gcore.jsdelivr.net/gh/Kele-Bingtang/static/img/Redis/20211226210314.png)
...
![image-20211226210814480](https://gcore.jsdelivr.net/gh/Kele-Bingtang/static/img/Redis/20211226210815.png)
...
![image-20210409150628118](https://gcore.jsdelivr.net/gh/Kele-Bingtang/static/img/Redis/20211226211059.png)
...
![](https://gcore.jsdelivr.net/gh/Kele-Bingtang/static/img/Redis/20211226211245.png)
...
![在这里插入图片描述](https://gcore.jsdelivr.net/gh/Kele-Bingtang/static/img/Redis/20211226211701.png)

此时哨兵监视着我们的主机 6379，当我们断开主机后：

![在这里插入图片描述](https://gcore.jsdelivr.net/gh/Kele-Bingtang/static/img/Redis/20211226211710.png)
